...
2. **能证明自己写对了**。`nmt/verify.py` 把权重搬进官方 `nn.Transformer`，逐层对比输出；
   `tests/` 里有 61 个不变量测试（因果性、KV cache 一致性、掩码、BLEU……）。
...
（下表在 4090 上补全；本地 4GB 显卡只能跑 small 配置做冒烟验证。）

| 模型 | 训练数据 | dev BLEU | test2019 BLEU | chrF | 长度比 | 训练时长 |
| --- | --- | --- | --- | --- | --- | --- |
| tiny（d_model=64，2+2 层） | 200 句合成 | 见测试 | — | — | — | 秒级 |
| small（d_model=256，3+3 层） | — | — | — | — | — | — |
| base（d_model=512，6+6 层） | — | — | — | — | — | — |

> 参考量级：base 配置在 10 万句对、约 4 小时训练后，newstest2019 的 BLEU 通常在 **12~18** 之间。
> 论文级别的 40+ BLEU 需要上千万句对和多卡训练 —— 这个项目的目的不是刷分，是让你看懂每个环节。
