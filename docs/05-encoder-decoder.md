...
x → 因果自注意力（只能看已经写出来的部分）  → 残差+Norm
  → 交叉注意力（Q 来自中文，K/V 来自英文 memory）→ 残差+Norm
  → 前馈网络                                → 残差+Norm
```

**交叉注意力就是"翻译"发生的地方**：每写一个中文字，它都在问"英文原句里哪几个词跟我现在写的这个字有关"。
...
| 解码器自注意力 | 中文 | 中文 | padding + 因果 |
| 解码器交叉注意力 | 中文 | **英文 memory** | padding（英文侧） |
...
以 `d_model=64`、词表 300、英文 7 个 token、中文 5 个 token 为例：
...
编码器把英文读成"上下文化的向量"，解码器拿着它一个词一个词写中文，
