...
把一句英文变成一句中文。中间要经过这些步骤，每一步都有对应的文件和章节：

```
原始语料（OPUS 上的平行句对）
   │  ① 下载 + 清洗           corpus.py           → 第 1 章
   ▼
干净的句对（英文一句、中文一句，一一对应）
   │  ② 训练分词器 + 编码      bpe.py / corpus.py  → 第 2 章
   ▼
整数 id 序列（"他明天去学校" → [1, 306, 88, ...]）
...
模型输入张量 src [B,S] / tgt [B,T]
   ...
   │     解码器（一边看 memory 一边写中文）decoder.py
   │     掩码（哪些位置不许看）        masks.py
   ▼
logits [B, T, 词表大小]
...
* **英译中**：中英的语序差异大（定语、状语位置完全不同），比英法、英德更能看出注意力在干什么；而且中文没有空格，分词器要真的处理一件"额外的事"。
* **新闻域**：WMT 官方每年都发布 newsdev / newstest 测试集，你的 BLEU 可以直接和论文、开源模型、商业系统的公开分数对比。自己随手切一个测试集，分数再高也不知道算高还是低。
