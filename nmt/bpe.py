...
英德两种语言都是"空格分词 + 拉丁字母"，所以 pre_tokenize 只需要处理三件事：

* 按空格切出"词"，标点单独成 token；
* 字母串里的连字符和撇号要跟着词走（E-Mail、don't、well-known 算一个词）；
* 数字（含小数、千分位）单独成一类。

对德语来说 BPE 特别值得：德语靠复合词造词（Wirtschaftswachstum = Wirtschaft + Wachstum），
词表里永远装不下所有复合词。BPE 会把它们拆成有意义的片段，
模型只需要认识 Wirtschaft 和 Wachstum，就能处理没见过的组合。
这也正是"子词"相对"整词"的核心优势。
...
为什么需要词尾标记 `</w>`？
    ...
    这样做还有个附带好处：合并规则自然学到 "en</w>" "ung</w>" "e</w>" 这类词尾片段，
    德语的名词后缀、动词变位、形容词词尾都能被单独建模 ——
    对形态丰富的语言，这是 BPE 比整词表更强的地方。
...
# 文本切分
# --------------------------------------------------------------------------
# 汉字（含扩展 A 区与兼容区）只用来做数据体检：英德语料里不该出现汉字，
# 一旦出现，多半是抓取错位或者编码坏了。
_CJK = r"\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff"
_CJK_CHAR = re.compile(rf"[{_CJK}]")

# 三类候选，按优先级排列：
#   1. 字母词（含连字符 / 撇号）   don't、E-Mail、well-known
#   2. 数字（含小数 / 千分位）     3.14、1,000
#   3. 其它任意单个非空白字符（标点、符号、残留的外文字）
# 注意 [^\W\d_] 是"Unicode 字母"，所以 ä ö ü ß é 这些字母都会被正确识别成一个词。
_TOKEN_RE = re.compile(r"[^\W\d_]+(?:['’-][^\W\d_]+)*|\d+(?:[.,]\d+)*|\S")

# 需要清掉的不可见字符（零宽空格、BOM、行分隔符等）
_CONTROL_RE = re.compile(r"[\x00-\x08\x0b\x0c\x0e-\x1f\x7f\u200b-\u200f\u2028\u2029\ufeff]")

# 词尾标记：加在每个"词"最后一个字符后面，用来还原词边界
WORD_END = "</w>"


def _word_to_symbols(word: str) -> List[str]:
    """把一个"词"变成 BPE 的初始符号序列：最后一个字符带上 `</w>` 词尾标记。"""
...
def normalize_text(text: str) -> str:
    ...
    用 NFC 而不是 NFKC：NFKC 会把全角字母、连字（ﬁ -> fi）也折叠掉，
    对德语正文没必要，而且会让原文里的专有名词产生意料之外的变化。
    """
...
class BPE:
    ...
    def detokenize(self, pieces: Sequence[str]) -> str:
        ...
        步骤：
          ...
          2. 标点两侧不该有空格，再按规则收一遍尾。

        已知的小瑕疵：引号的左右两半在 Unicode 里是不同字符，
        而 "„……“"（德式）和 "“……" "（英式）用法又不一样，这里不做智能判断，
        所以引号旁偶尔会多一个空格。BLEU 的 13a 分词会把标点整个去掉，
        这个瑕疵不影响指标，只影响肉眼观感。
        """
